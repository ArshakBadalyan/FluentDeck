...
│    matheapp-f/   →  optional (Flutter web profile)          │
└────────────────────────────────────────────────────────────┘
...
- GitHub access to `strapi-math` (and `matheapp-f` if using web/mobile staging config)
- SSH key
...
# git clone https://github.com/sahakyan-dev/matheapp-f.git matheapp-f
...
If `DEPLOY_WEB=true`, add the **same** `.pub` to **matheapp-f** → Deploy keys.
...
See `matheapp-f/deploy/CLIENT-STAGING.local.md` if present.

### Flutter web on same VPS (optional)

```bash
cd /opt/mathapp-staging/matheapp-f
cp .env.staging.example .env.staging
